...
exports.crearPedido = async (req, res) => {
    ...
    await axios.post(`http://localhost:3001/restaurante/crearPedido`, { pedido });
...
exports.obtenerPedidoRestaurante = async (req, res) => {
    const { id } = req.params;
    const respuestaRestaurante = await axios.get(`http://localhost:3001/restaurante/obtenerPedido/${id}`);
...
exports.obtenerPedidoRepartidor = async (req, res) => {
    const { id } = req.params;
    const respuestaRepartidor = await axios.get(`http://localhost:3000/repartidor/obtenerPedido/${id}`);
